...
def check_with_gigachat(text, token):
    ...
    prompt = f"""
Ты строгий, но справедливый российский преподаватель (вуза или школы). 
Оцени работу студента/школьника по следующим критериям и дай **подробный, но понятный** отчёт:

**1. Общая оценка работы** (от 1 до 5 баллов) + короткое резюме

**2. Структура работы:**
- Введение: актуальность, цель, задачи, объект, предмет — ✅ / ⚠️ / ❌ + комментарий
- Основная часть: логичность, наличие глав/подразделов
- Заключение: выводы, итоги, соответствие цели
- Список литературы: оформление по ГОСТ, количество источников

**3. Содержательные замечания:**
- Цель и задачи чётко сформулированы?
- Объект и предмет исследования правильно определены?
- Есть ли практическая/эмпирическая часть?
- Логика изложения и связность текста
- Объём работы (достаточно ли материала?)

**4. Конкретные рекомендации:**
Что нужно исправить в первую очередь (максимум 5 пунктов)
...
if uploaded_file is not None:
    ...
    try:
        ...
        if st.button("🔍 Проверить через GigaChat + оформить по ГОСТ"):
            ...
            st.subheader("📊 Подробный отчёт GigaChat по работе")

            # Красивый дизайн отчёта
            st.markdown("---")
            st.markdown(ai_report)
            st.markdown("---")

            st.caption("💡 Рекомендация: Исправь замечания, отмеченные ⚠️ и ❌, перед сдачей работы.")
